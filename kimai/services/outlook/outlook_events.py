...
PREFERRED_USERNAME = os.getenv(
    "OUTLOOK_MSAL_USERNAME"
)  # optional hint like you@work.com
...
def save_cache(c: msal.SerializableTokenCache):
...
def pick_account(app: msal.PublicClientApplication, preferred: Optional[str] = None):
...
def get_token() -> str:
    ...
    account = pick_account(app, PREFERRED_USERNAME)
...
def fetch_calendar_view(token: str, start: dt.datetime, end: dt.datetime) -> List[Dict]:
    headers = {
...
def main():
    ...
    token = get_token()
    events = fetch_calendar_view(token, now, now + dt.timedelta(days=7))
